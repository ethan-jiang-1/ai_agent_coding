# 13 高级 Agentic 协调：先分 subtask、共享 backend、外部 automation 和 handoff 层
...
- 外部 runner 上的 durable automation
...
- 需要定时 review、issue triage、PR automation 这类 durable workflow，用 GitHub runner 等外部 automation surface。
...
3. 这是不是已经进入外部 durable automation
4. 这是不是要给别人看、给别人接，还是要导入导出状态
5. 这是不是已经超出 OpenCode 原生能力，要借外部 Git 或别的调度系统

## OpenCode 的特殊边界

- subagent / subtask 是正式能力
- `serve` / `web` / `attach` 是共享 backend，不是 scheduler
- GitHub integration 等 durable automation surface 是外部 runner，不是 native shared backend 的延伸
...
- 不要把 GitHub runner 上的自动化回写成 OpenCode 原生 scheduler。
- 不要把 share/export/import 混成同一种状态传递。
- 不要把 absence of worktree / background jobs 硬补成存在的功能。

## 验收标准

- 你能明确说出当前依赖的是 subtask、shared backend、external automation，还是 share/export/handoff。
