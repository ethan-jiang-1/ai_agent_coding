...
- 如果根规则已经过时或太薄，不要继续每次口头解释项目背景；`/init` 会扫描仓库、必要时追问几个关键问题，并就地改进现有 `AGENTS.md`。
...
- 一旦 `/init` 在项目里生成了 `AGENTS.md`，后续 session 的项目规则入口就会从 `CLAUDE.md` fallback 切到 `AGENTS.md`
...
- 不要把任务计划写进 `AGENTS.md`。
- 不要把所有外部规则全文复制进根规则。
- 不要一边运行过 `/init` 生成 `AGENTS.md`，一边还按旧习惯以为项目主要靠 `CLAUDE.md` 生效。
...
- 你知道当前项目规则的真正主入口到底是 `AGENTS.md` 还是 fallback 的 `CLAUDE.md`。
