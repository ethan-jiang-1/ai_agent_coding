...
- `Commands` 页列出的 quick commands
- 其他单独文档里出现的 chat commands，例如 `/summarize`
- `.cursor/commands`
- `~/.cursor/commands`

也就是说，editor 的丰富度一部分来自内建 command，另一部分来自“可扩展命令面”，而不只是很长的一串官方内建 slash。
...
3. 对控制动作和可复用 workflow 这类不适合硬塞进原章节的内容，单独落到第 `12` 章交互控制层
...
### 1. 内建 quick commands 与已文档化 chat commands

官方 `Commands` 页面搜索摘要可确认，Cursor 支持通过 `/command` 调用 quick commands；另外 summarization 页面还能确认手动 `/summarize`。当前至少能确认这些例子：
...
| `/summarize` | 手动触发长对话总结 | C | `commands/06` |
...
- 但官方文档和搜索摘要没有保证这就是完整清单，所以不要把这些例子写成“全部 slash commands”
...
这一组更适合作为 Type D / interactive control-layer 内容，进入独立的第 `12` 章。
...
### 4. summarization 也是官方上下文管理层

Cursor 官方 summarization 页面还能确认：

- 长对话会自动 summarization
- chat 里支持手动 `/summarize`

因此这次整理里，`commands/06` 不能只写 `/Reset Context` 和新开 tab。
...
### 3. `/summarize` 和 `/Reset Context` 不是一回事
...
- `/summarize`：压缩长对话并保留关键信息
- `/Reset Context`：更接近直接清掉当前上下文

`/Reset Context` 不应被误写成和 Claude Code `/compact` 对等。
...
### 5. `Manual` 不能被写成完全稳定的当前模式清单

当前更稳的官方 modes 页面，中心叙事是：

- `Agent`
- `Ask`
- `Custom`

但较早或其他语言页里，仍然能看到 `Manual`。

因此在 `practice_cursor/` 里更安全的写法应该是：

- 如果你的当前 Cursor 构建仍暴露 `Manual`，把它当成窄编辑通道
- 不要把它写成所有当前官方页都稳定列出的模式

## 对这次融合最重要的 no-mapping 内容

这次最值得进入独立第 `12` 章交互控制层的，是这类 interactive command surface：
...
| `03` | 可补 `setup-new-feature` 这类规划型 custom command，但不应把它写成基线要求 |
| `04` | 已有 `/Generate Cursor Rules`，并且适合继续保留 |
| `05` | 已有 `/Add Open Files to Context`、`/Add Active Files to Context` |
| `06` | 应写入自动 summarization、`/summarize`、`/Reset Context`、`/Disable Iterate on Lints` 的边界 |
| `07` | 没发现 editor 内建 `/model`；模型选择仍以 model picker / custom modes 为主 |
| `08` | 可补 `security-audit`、`code-review-checklist`、`run-all-tests-and-fix`，但它们更像官方示例而不是统一基线 |
| `09` | history / export / `@Past Chats` 已补入 |
| `10` | 没发现 background-agent 专用 built-in slash；`create-pr`、`address-github-pr-comments` 只能作为可选交接示例 |
| `11` | 没发现 editor 侧成本 / usage slash command；这一章仍以 dashboard / spending limit 为主 |
| `12` | 继续承接 project/global commands 这层交互控制面，但不要把 commands 写成必须依赖的层 |

## 对 `practice_cursor/` 的落地策略

1. 先把高相关 quick commands 融入 `commands/04`、`05`、`06`
2. 再把 history / export / `@Past Chats` 的交互入口补强到 `commands/09`
3. 新增第 12 章，作为交互控制层章节，承接 project/global custom commands 等内容
