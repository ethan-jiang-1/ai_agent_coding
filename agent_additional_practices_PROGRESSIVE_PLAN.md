...
- `<tool>/validation/`
  - 每个活跃候选默认 1 个 candidate card 文件，文件名直接使用 `<candidate_id>.md`
  - 如果某个候选触发定向深挖，再额外补 `YYYY-MM-DD_<candidate_id>_deep_dive.md`
- `<tool>/summary/`
  - 每个工具结束时固定产出 1 份 exit summary
- `90_review_queue/promote/`、`merge/`、`hold/`、`reject/`
  - 每个进入该状态的候选默认 1 份 decision memo，文件名直接使用 `<candidate_id>.md`
- `90_review_queue/apply_matrix/`
  - 每一轮 run 默认 1 份 run-level apply matrix

补充约束：

- `candidate_id` 一旦分配，不因候选改名而更换文件名
...
- 初始化 `INDEX.md`
- 初始化 `STATUS_LOG.md`
- 记录本轮执行日期快照与有效主时间窗；如果这是重启 run，还要注明与上一次 run 的关系
...
- 在 candidate card 中写清 `strong_tool`、`support_level`、主要支撑证据和主要反证
