...
**全局规则保持极度精简（≤800 Token / 60 行），目录规则按路径精确附着；重型工作流则按工具实际能力拆到 Skills / Subagents / Custom Modes，而不是塞进一个巨大的全局文件。**
...
| 层级 | 典型载体 | 加载时机 | 适合存放的内容 |
|------|---------|---------|--------------|
| 全局层 | `CLAUDE.md` / `AGENTS.md` / always-apply 的 `.cursor/rules/*.mdc` | 会话启动或进入工作区时 | Git 流程、技术栈基线、禁止事项（≤800 Token） |
| 路径层 | `.claude/rules/*.md` / `.cursor/rules/*.mdc` / 更深层的 `AGENTS.md` | 访问特定目录或文件时附着 | 框架特定规范、目录级约束 |
| 专项工作流层 | Codex Skills / Claude subagents / Cursor Custom Modes / Aider 的专用 prompt 文件 | 需要时显式调用 | 复杂工作流、专项任务模板 |
...
以 Codex Skills 为例，专项工作流可以写成独立的 `SKILL.md`：
```markdown
...
同样的思想，在其他已核验工具里要换成它们自己的正式机制：

- Claude Code：更适合放进 subagent 指令文件
- Cursor：更适合做成 Custom Mode 或独立的审查工作流
- Aider：更适合放进可复用的 prompt 文件，再配合 `--read` / `/ask` / `architect`

### 各工具的规则文件对应关系

| 工具 | 全局层 | 路径层 | 技能层 |
|------|--------|--------|--------|
| Claude Code | `CLAUDE.md` | `.claude/rules/*.md`（YAML frontmatter） | subagents |
| Cursor | `AGENTS.md` 或 always-apply 的 `.cursor/rules/*.mdc` | `.cursor/rules/*.mdc` 的 `globs` / 类型 | Custom Modes / Background Agents |
| Aider | 项目约定文档（通过 `--read` / `/read` 引入） | `--read` 精确加载特定文件 | `/ask` / `architect` / 专用 prompt 文件 |
| Codex CLI | `AGENTS.md` | 更深层的 `AGENTS.md` 目录覆盖 | Skills / subagents |
...
**真实后果**：一个包含 11,000 Token 的 always-apply `.cursor/rules/*.mdc` 规则集合，即使用户只是让 AI "修改按钮颜色"，系统也可能先带入这批全局规则。这不只是浪费 Token，更会导致"注意力稀释"。AI 在处理简单的 CSS 问题时，脑子里还装着数据库迁移规范和 API 安全审计要求，注意力被无关内容分散。
...
一个团队同时使用 Cursor（前端）和 Claude Code（后端），各自维护 `.cursor/rules/*.mdc` 和 `CLAUDE.md`，内容不同步。
...
精简的、稳定的全局规则文件，更容易形成可复用的稳定前缀。对支持提示词缓存的平台来说，这通常会降低重复加载规则层的边际成本；反之，一个内容频繁变动的大型规则文件，会不断破坏前缀复用。
...
AI 能根据这个路径去读取对应文件，不需要把内容内联到规则文件里。路径锚点既保持规则文件轻量，又确保 AI 看到的是最新的代码。

### 跨工具同步：单一规则源

对于同时使用多个 AI 工具的团队，可以维护一份单一规则源，再同步到各工具的正式载体：
...
3. **单一规则源的实际采用情况**：在多工具团队中，用一份中立规则源再分发到 `CLAUDE.md`、`AGENTS.md`、`.cursor/rules/*.mdc` 的实际效果如何？有没有格式转换导致的语义损失？
