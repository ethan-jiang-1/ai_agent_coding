...
- 基准章节数不是固定的
- 不同 agent 可能少一项、等于或多于另一个 agent 的既有章节数
- 所以交互式命令面的兜底章，不应该被硬编码成固定的 `12`
- 正确理解是：**当前基准章节数 `N` 之后的最后补充章，也就是 `N+1`**

例子：

- 如果当前 agent 有 11 章，兜底章就是第 12 章
- 如果当前 agent 有 10 章，兜底章就是第 11 章
- 如果当前 agent 有 12 章，兜底章就是第 13 章
...
## Final Supplemental Chapter Rule
...
这时应该新增一个**最后的补充兜底章**，作为交互式命令面的兜底层。
...
只在确实必要时，为 interactive-only 的命令面补一个最后的补充兜底章。

验收：

- 如果新增了补充兜底章，它收的确实是无法自然并入现有基准章节的内容
- 如果没有新增补充兜底章，也能解释为什么现有章节已经足够承载

具体执行：

- 先尝试融入现有基准章节
- 只有在映射类型长期是 `No mapping` 且内容重要时，才新增编号为 `N+1` 的最后补充章
