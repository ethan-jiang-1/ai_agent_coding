...
- 基准章节数不是固定的
- 不同 agent 的基准章节数，可能少于、等于或多于别的 agent
- 所以交互式命令面的补充控制层章节，不应该被硬编码成固定的 `12`
- 正确理解是：**当前基准章节数 `N` 之后的最后一个补充控制层章节，也就是 `N+1`**

例子：

- 如果当前 agent 有 11 章，补充控制层章节就是第 12 章
- 如果当前 agent 有 10 章，补充控制层章节就是第 11 章
- 如果当前 agent 有 12 章，补充控制层章节就是第 13 章
...
## Supplemental Control-Layer Chapter Rule

大多数情况下，应该优先把交互式命令面融合进当前 agent 已有的基准章节，而不是轻易新增章节。

但如果某些交互式命令同时满足：

- 很重要
- 没有 external 命令对应
- 也很难自然并入当前 agent 的既有章节

那就不要硬塞。

这时应该新增一个**最后的补充控制层章节**，作为交互式命令面的独立承载层。

这个章节的编号应当跟着当前 agent 的基准章节数变化：

- 基准章节数是 `N`
- 补充控制层章节就是 `N+1`

对当前有 11 章的 agent，它自然是第 12 章；但这不是固定写死的规则。

### The supplemental control-layer chapter is for

- interactive-only 的重要命令
- 没有 external 对应物的 slash commands
- 很难自然并入当前基准章节，但对真实工作流很有价值的会话内操作

### The supplemental control-layer chapter is not for

- 其实能融入现有章节、只是需要多写一句映射说明的内容
- 只是某个外部命令的会话内别名
- 只是组合对应，但没有必要独立成章的内容
...
### Step 4. Add the final supplemental chapter only if needed

目标：

只在确实必要时，为 interactive-only 的命令面补一个最后的补充控制层章节。

验收：

- 如果新增了补充控制层章节，它收的确实是无法自然并入现有基准章节的内容
- 如果没有新增补充控制层章节，也能解释为什么现有章节已经足够承载

具体执行：

- 先尝试融入现有基准章节
- 只有在映射类型长期是 `No mapping` 且内容重要时，才新增编号为 `N+1` 的最后补充控制层章节
